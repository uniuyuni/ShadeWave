...
- グローバル補正、ローカルマスク、ジオメトリ補正、Film Process/ルック系効果、ノイズ除去、デヘイズ、シャープ、グレイン、ビネットを適用する。
...
| RAW | `.cr2`, `.cr3`, `.crw`, `.nef`, `.nrw`, `.arw`, `.dng`, `.orf`, `.raf`, `.rw2`, `.sr2`, `.pef`, `.raw`, `.3fr`, `.fff`, `.erf`, `.kdc`, `.dcr`, `.mrw`, `.rwl`, `.srw`, `.mef` |
| HDR / シーンリニア | `.exr` |
...
- ローカル編集: マスクレイヤー、合成マスク、フリードローマスク、Quick Select系のエッジリファイン、マスクジオメトリ、Photoshop風のブレンドモード(乗算・スクリーン・オーバーレイ・ソフトライト等)を持つDraw Effects、エクスポート時のヘッドレスマスク再生。
- AI支援マスク: SAM3の矩形/テキストセグメンテーション、Depth Pro深度マスク、顔マスク。
- クリエイティブルック: Film Process、レンズシミュレーション、クロスフィルター、グロー、オートン、グレイン、ビネット、単色オーバーレイ。
...
| `display_color_gamut` | `auto` | 表示変換のターゲット |
| `cat` | `cat16` | 色順応変換 |
...
Shade Wave は GNU General Public License version 3 or later で公開します。詳細は `LICENSE` を参照してください。
...
- リポジトリ: https://github.com/uniuyuni/ShadeWave
