...
| `PLATYPUS_GEOMETRY_DEFERRED_CACHE` | 有効 | deferred preview transform（行列 + MLS coarse map）のフレーム間キャッシュ。`0` で毎フレーム再構築の旧挙動に戻す。 |
...
| `PLATYPUS_IMAGE_TRANSFORM_MESH_DENSE` | 無効 | fused preview の mesh 変位を GPU 常駐の密マップに bake してフレーム間再利用する。出力は off とビット一致。ズーム/パン等 mesh 不変の再描画が速くなる。 |
| `PLATYPUS_IMAGE_TRANSFORM_MESH_DENSE_BUDGET_MB` | `256` | 密マップの上限サイズ (MB)。超える region では bake せず LUT 経路へフォールバックする。 |
...
| `PLATYPUS_COATING_BACKEND` | `auto` | Lens Simulator の coating backend。`metal`, `reference`(`python`/`opencv`) などで戻せる。 |
| `PLATYPUS_COATING_METAL_STRICT` | 無効 | Coating Metal失敗時にfallbackせず例外を投げる。 |
| `PLATYPUS_LENS_ABERRATION_BACKEND` | `auto` | Lens Simulator の収差backend。`metal`, `reference`(`python`/`opencv`) などで倍率色収差のMetal経路を切り替える。 |
| `PLATYPUS_LENS_ABERRATION_METAL_STRICT` | 無効 | Lens aberration Metal失敗時にfallbackせず例外を投げる。 |
| `PLATYPUS_LENS_EFFECT_BACKEND` | `auto` | Lens Simulator 内サブ効果backend。ボケ色縁/形状ボケ/渦ボケ/サンスターを `effect_backends` 経由で処理する。`auto` ではボケ色縁とサンスターをMetal化し、形状ボケは現状OpenCVの方が速いためreferenceを使う。`metal` 明示時は形状ボケ/渦ボケMetal検証経路も使う。未native化サブ効果はreferenceへfallbackする。 |
| `PLATYPUS_LENS_EFFECT_METAL_STRICT` | 無効 | Lens effect Metal失敗時にfallbackせず例外を投げる。 |
...
| `PLATYPUS_LUT_BACKEND` | `auto` | 3D LUT backend。`metal`, `reference`(`python`/`numpy`) などでNumPy参照実装へ戻せる。 |
| `PLATYPUS_LUT_METAL_STRICT` | 無効 | LUT Metal失敗時にfallbackせず例外を投げる。 |
...
| `PLATYPUS_DEBUG_LIQUIFY` | 無効 | Liquify（メッシュ変形）のdebug log。 |
