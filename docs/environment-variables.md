...
| `PLATYPUS_SAM3_COREML_BACKBONE` | 無効 | SAM3 image backbone を Core ML 経由に差し替える。macOS app runtimeでは既定有効にされる。 |
| `PLATYPUS_SAM3_COREML_BACKBONE_MODEL` | `checkpoints/sam3_backbone.mlpackage` または `.mlmodelc` | Core ML backbone model の明示パス。 |
| `PLATYPUS_SAM3_COREML_AUTO_CONVERT` | 有効 | Core ML model が無い場合に自動変換を試みる。 |
| `PLATYPUS_SAM3_COREML_RETRY` | 無効 | Core ML backbone failure 後も再試行する。 |
| `PLATYPUS_SAM3_COREML_COMPUTE_UNITS` | `cpu_and_gpu` | Core ML compute unit。`all`, `cpu`, `gpu`, `cpu_and_gpu`, `ne`, `cpu_and_ne`。 |
| `PLATYPUS_NAFNET_RESTORE_LOW_FREQUENCY` | 無効 | NAFNet helper の低周波復元オプション。 |
| `NAFNET_APPLESILICON_ROOT` | 未指定 | Apple Silicon版NAFNet helper の外部root。 |
...
| 変数 | 既定 | 用途 |
| --- | --- | --- |
| `PLATYPUS_SPLASH_SCREEN` | 無効 | 起動時に `assets/Shade Wave.png` のスプラッシュスクリーンを表示する。 |
