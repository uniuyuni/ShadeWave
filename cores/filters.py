...
def apply_lensblur(
    ...
    # depth_mapがNoneの場合は全体を背景ぼけとして扱う
    if depth_map is None:
        # 全体を最奥（0.0）として扱い、ピント面を手前（1.0）に設定
        # これにより全体が均一に背景ぼけになる
    ...
    # CoC（Circle of Confusion）半径マップを計算
    # 前景と背景で特性を変える
    is_foreground = depth_smooth > focus_depth
    
    # 前景ぼけはより強く、背景ぼけはより柔らかく
    ...
    coc_radius = np.clip(coc_radius, 0, max_coc_radius)
    
    # ぼかしレベルをfloatで計算（線形補間用）
    blur_level_float = coc_radius * (num_levels - 1) / max_coc_radius
