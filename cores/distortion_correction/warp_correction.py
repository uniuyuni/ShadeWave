...
def apply_homography_to_tcg_point(
    x_tcg: float,
    y_tcg: float,
    H: np.ndarray,
    image: np.ndarray,
    tcg_info: Optional[Dict] = None
) -> Tuple[float, float]:
    """
    TCG座標系の点にホモグラフィ変換を適用
    
    TCG座標 → ピクセル座標 → ホモグラフィ変換 → TCG座標
    の一連の変換を行う便利な関数
    ...
    -------
    Tuple[float, float]
        変換後のTCG座標 (x_tcg', y_tcg')
        
    Examples
    --------
    >>> # 補正を実行
    >>> corrected, H = correct_with_lines(image, reference_lines)
    >>> 
    >>> # TCG座標(-0.5, 0.3)の点を補正後のTCG座標に変換
    >>> if H is not None:
    ...     x_new, y_new = apply_homography_to_tcg_point(-0.5, 0.3, H, image)
...
def apply_inverse_homography_to_tcg_point(
    x_tcg: float,
    y_tcg: float,
    H: np.ndarray,
    image: np.ndarray,
    ...
    """
    TCG座標系の点に逆ホモグラフィ変換を適用
    
    補正後のTCG座標を元のTCG座標に戻す
    ...
    >>> corrected, H = correct_with_lines(image, reference_lines)
    >>> 
    >>> # 補正後のTCG座標(0.2, -0.3)を元のTCG座標に戻す
    >>> if H is not None:
    ...     x_orig, y_orig = apply_inverse_homography_to_tcg_point(0.2, -0.3, H, image)
    ...     print(f"補正後TCG(0.2, -0.3) → 元のTCG({x_orig:.2f}, {y_orig:.2f})")
    """
    # 逆行列を計算
    H_inv = np.linalg.inv(H)
    
    # 逆変換を適用
    return apply_homography_to_tcg_point(x_tcg, y_tcg, H_inv, image, tcg_info)
