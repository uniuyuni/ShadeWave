...
def _deserialize_ai_mask_bitmap(pipeline, d):
    """AI マスク(Segment/Face/TargetText)の image_mask_cache をストア経由で復元する。
    旧形式(インライン圧縮)はデコードしてストアへ移行し、新形式(キーのみ)はストアから引く。
    GUI 側 widgets/mask_editor2.py BaseMask._deserialize_image_mask_cache と対称。
    戻り値: (image_mask_cache, image_mask_cache_key)"""
    legacy_inline = d.get("image_mask_cache", None)
    key = d.get("image_mask_cache_key", None)
    if legacy_inline is not None:
        ...
        putter = getattr(pipeline, "put_ai_mask_bitmap", None)
        if callable(putter):
            putter(key, image)
        return image, key
    if key is not None:
        getter = getattr(pipeline, "get_ai_mask_bitmap", None)
        if callable(getter):
...
def _get_or_put_ai_mask_bitmap(pipeline, cache_key, compute_func):
    """共有ストアに同一キーの結果が既にあれば推論をスキップして返す。
    GUI 側 BaseMask._get_or_compute_image_mask_cache と対称(ヘッドレスは単一スレッド想定のため
    pending 制御は省略)。"""
    getter = getattr(pipeline, "get_ai_mask_bitmap", None)
...
class HeadlessSegmentMask:
    def __init__(self, ctx, pipeline=None):
        self.ctx = ctx
        self.pipeline = pipeline
        self.name = "Segment"
        self.effects = effects.create_effects()
    ...
    def deserialize(self, d):
        ...
        self.image_mask_cache, self.image_mask_cache_key = _deserialize_ai_mask_bitmap(
            self.pipeline, d
        )
    ...
    def get_mask_image(self):
        ...
        if (
            ...
            segment_mask = _get_or_put_ai_mask_bitmap(
                self.pipeline,
                cache_key,
                lambda: inference_runtime.predict_sam3_bbox(img, [min_x, min_y, w, h], False),
            )
...
class HeadlessFaceMask:
    ...
    def get_mask_image(self):
        ...
        if (
            ...
            self.image_mask_cache_key = cache_key
            img = self.ctx.get_original_image_rgb()
            faces_mask = _get_or_put_ai_mask_bitmap(
                self.pipeline,
                cache_key,
                lambda: inference_runtime.predict_face_mask(img, exclude_names),
            )
    ...
    def get_mask_image(self):
        ...
        if (
            ...
            segment_mask = _get_or_put_ai_mask_bitmap(
                self.pipeline,
                cache_key,
                lambda: inference_runtime.predict_sam3_text(img, text, False),
            )
            self.image_mask_cache = segment_mask
...
def instantiate_mask_from_type(ctx, pipeline, mask_type: str):
    ...
    if mt == MaskTypeStr.SEGMENT:
        return HeadlessSegmentMask(ctx, pipeline)
    if mt == MaskTypeStr.DEPTHMAP:
        return HeadlessDepthMapMask(ctx, pipeline)
    if mt == MaskTypeStr.FACE:
        return HeadlessFaceMask(ctx, pipeline)
    if mt == MaskTypeStr.TARGET_TEXT:
        return HeadlessTargetTextMask(ctx, pipeline)
